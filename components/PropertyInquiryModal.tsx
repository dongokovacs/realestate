...
export default function PropertyInquiryModal({
  isOpen, onClose, propertyId, propertyTitle, propertyLocation,
  ...
  useEffect(() => {
    ...
    else (triggerRef.current as HTMLElement)?.focus?.();
  }, [isOpen]);
        ...
        <>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="presentation">
            <motion.div
              ...
              style={{ background: "linear-gradient(135deg, rgba(139,94,60,0.3) 0%, rgba(59,114,232,0.2) 100%)" }}
            >
              <div
              ...
              >
                ...
                {status === "success" ? (
                  <div className="text-center py-6" role="status" aria-live="polite">
                    ...
                    <h2 id={titleId} className="heading-display text-off-white text-2xl mb-2">Inquiry Sent!</h2>
                    <p className="text-off-white/50 text-sm font-bold mb-6">
                      A Haven specialist will be in touch about{" "}
                      <span className="text-brown-light">{propertyTitle}</span> within 24 hours.
                    </p>
                    <button
                      onClick={handleClose}
                  ...
                  <>
                    <div className="mb-6">
                      ...
                      <h2 id={titleId} className="heading-display text-off-white mt-1 mb-0.5" style={{ fontSize: "1.5rem" }}>
                        {propertyTitle}
                      </h2>
                    ...
                    <form ref={formRef} onSubmit={handleSubmit} noValidate>
                      ...
                      <div className="mb-3">
                        <label htmlFor="inq-name" className={labelClass}>Teljes név <span aria-hidden="true">*</span></label>
                        <input
                          id="inq-name" name="full_name" type="text"
                      ...
                      <div className="mb-3">
                        <label htmlFor="inq-email" className={labelClass}>E-mail cím <span aria-hidden="true">*</span></label>
                        <input
                      ...
                      <div className="mb-3">
                        <label htmlFor="inq-message" className={labelClass}>Üzenet</label>
                        <textarea
                          id="inq-message" name="message"
                          placeholder="Érdeklődsz az ingatlan iránt, szeretnél megtekinteni…"
                          rows={3}
                          disabled={isPending} className={`${inputClass} resize-none`}
                        />
                      </div>

                      {status === "error" && (
                        <p role="alert" className="text-red-400 text-xs font-bold mb-3">{errorMsg}</p>
                      )}

                      <button
                        type="submit"
                        disabled={isPending}
                        className="group w-full flex items-center justify-center gap-2 bg-brown hover:bg-brown-light text-white font-black text-[11px] uppercase tracking-[0.15em] px-6 py-4 rounded-xl transition-all duration-400 disabled:opacity-50 disabled:cursor-not-allowed"
                        style={{ transitionTimingFunction: "cubic-bezier(0.16,1,0.3,1)" }}
                        aria-busy={isPending}
                      >
                        {isPending ? "Küldés…" : "Érdeklődés küldése"}
                        {!isPending && (
